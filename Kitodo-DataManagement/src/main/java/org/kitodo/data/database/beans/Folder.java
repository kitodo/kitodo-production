...
/**
 * A type of folder in the process directories of processes in this project.
 * Typically, a folder has a corresponding directory in the process directory of
 * each processes which can be located by appending {@link #path} to the process
 * directory, and a {@code <fileGrp>} structure in the produced METS file with
 * the {@code USE} attribute set to the value of {@link #fileGroup}, and links
 * to the files contained in the directory formed by concatenating the
 * {@link #urlStructure} with the simple name of the file. However, a “folder”
 * can also only exist on the drive without being exported to METS, or it can
 * exist only virtually without correspondence on a drive, just to produce the
 * METS {@code <fileGrp>} structure. The assumption in that case is, that each
 * folder contains the same number of files with the same names, except for the
 * file extension, which can vary. This structure is used to represent different
 * versions of the same object (a small low quality JPEG thumbnail, a high
 * quality JPEG, an OCR-processed PDF, etc.) The different variants can be
...
public class Folder extends BaseBean {
     ...
     * java.net.URI, org.kitodo.api.imagemanagement.ImageFileFormat)}. The
     * value is the factor of scaling for the derivative, a value of 1.0
