	...
	public static GoobiProcess getProcessByPPN(IdentifierPPN PPN) {
		...
		try {
			...
			Criteria criteria = session
					...
					.add(Restrictions.or(Restrictions.eq("we.titel", "PPN digital a-Satz"),
							Restrictions.eq("we.titel", "PPN digital f-Satz")))
					.add(Restrictions.eq("ve.titel", "Titel"))
					.add(Restrictions.eq("we.wert", PPN.toString()))
					.addOrder(Order.asc("we.wert"))
					.setProjection(
							Projections.projectionList().add(Projections.property("we.wert"), "identifier")
	...
	public static List<GoobiProcessStep> getAllProcessSteps(IdentifierPPN PPN) {
		...
		try {
			...
			Criteria criteria = session
					...
					.setProjection(
							Projections.projectionList().add(Projections.property("reihenfolge"), "sequence")
