...
/**
 * The class ActiveMQProcessor offers general services, such as making the incoming messages available
 * as MapMessages and publishing the results. When I came clear that this code would be necessary for every processor,
 * I thought an abstract class would be the right place for it. ActiveMQProcessor also provides a place to save
 * the checker for the ActiveMQDirector, to be able to shut it down later.
 * 
...
public abstract class ActiveMQProcessor implements MessageListener {
    ...
    /**
	 * Instantiating the class ActiveMQProcessor always requires to pass the name of the queue it should
     * be attached to. That means, your constructor should look like this:
	 * <pre>
	 ...
	 * </pre>
	 *
	 * <p>If the parameter is not set in goobi_config.properties, it will return “null” and so prevents it from
     * being set up in ActiveMQDirector.</p>
	 *
	 * @param queueName
	 *            the queue name, if configured, or “null” to prevent the processor from being connected.
	 */
    ...
    /**
	 * The method onMessage() provides a corset which checks the message being a MapMessage, performs a type safe
     * type cast, and extracts the job’s ID to be able to send useful results to the results topic, which it does after
	 * the abstract method process() has finished.
	 *
	 * <p>Since this will be the same for all processors which use MapMessages, I extracted the portion into
