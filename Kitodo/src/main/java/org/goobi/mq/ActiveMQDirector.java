 ...
 * that “activeMQ.hostURL” is configured in the goobi_config.properties file. To disable the service, the entry there
 * should be emptied or commented out. Otherwise, a valid configuration would not start working and an administrator
 * will hardly have a chance to find out why without inspecting the source code.</p>
 *
 * <p>The class ActiveMQDirector also provides a basic ExceptionListener implementation as required for the
 * connection.</p>
 *
...
public class ActiveMQDirector implements ServletContextListener, ExceptionListener {
	...
	/**
	 * The method contextInitialized() is called by the web container on startup and is used to start up the active MQ
	 * connection. All processors from services[] are registered.
	 *
	...
	/**
	 * Sets up a connection to an active MQ server. The connection object is global because it is needed later to shut
	 * down the connection.
	 *
	 * @param server should be “tcp://{host}:{port}” or “vm://localhost” in case that the server is run inside the same
	 *                  virtual machine
	 ...
	 *
	 * <p>If a queue name was configured for a service, a MessageConsumer is set up to listen on that queue and, in case
	 * of incoming messages, make the service process the message. The message checker is saved inside the service to
	 * be able to shut it down later.</p>
	 */
	...
	/**
	 * This sets up a connection to the topic the results shall be written to. The delivery mode is set so “persistent”
	 * to allow consumers not online with the server in the moment of message submission to read the messages later.
	 * The log messages are set to be kept on the server for 7 days. This value can be overridden from the
	 * goobi_config.properties parameter “activeMQ.results.timeToLive”. The time to live must be specified in
	 * milliseconds; 0 disables the oblivion.
	 * (See also: http://docs.oracle.com/javaee/6/api/javax/jms/MessageProducer.html#setTimeToLive%28long%29 )
	 *
	 * @param topic name of the active MQ topic
	...
	/**
	 * Any class that wants to create new Active MQ Messages needs read access to the session, since Active MQ messages
	...
	/**
	 * Instances of WebServiceResult can be sent by calling their send() method. Therefore, they need read access on
	 * their topic.
	 *
	...
	/**
	 * The method contextDestroyed is called by the web container on shutdown. It shuts down all listeners, the session
	 * and last, the connection.
	 *
