 ...
 * validated.
 */
public class MissingOrDamagedImagesFilterPredicate implements Predicate<UseFolder> {
     ...
     */
    @Override
    public boolean test(UseFolder folder) {
        ...
        if (!imageURI.isPresent()) {
            ...
            if (fileType.isPresent()) {
                KitodoServiceLoader<LongTimePreservationValidationInterface> serviceLoader = new KitodoServiceLoader<>(
                        LongTimePreservationValidationInterface.class);
                ValidationResult validated = serviceLoader.loadModule().validate(imageURI.get(), fileType.get());
