...
enum FilterField {
    ...
    TASK("EXISTS (SELECT 1 FROM Task task WHERE task.process = ~ AND task.title = :#)",
            "title", LikeSearch.NO, "tasks AS task WITH task.id", "id", null, null),
    TASK_AUTOMATIC("tasks AS task WITH task.typeAutomatic = :queryObject AND task.title",
            ...
            Boolean.TRUE, null),
    TASK_UNREADY("EXISTS (SELECT 1 FROM Task task WHERE task.process = ~ "
            + "AND task.processingStatus = :queryObject AND task.title = :#)",
    ...
    TASK_READY("EXISTS (SELECT 1 FROM Task task WHERE task.process = ~ "
            + "AND task.processingStatus = :queryObject AND task.title = :#)",
            "~.processingStatus = :queryObject AND ~.title", LikeSearch.NO,
            "tasks AS task WITH task.processingStatus = :queryObject AND task.id",
            "processingStatus = :queryObject AND id", TaskStatus.OPEN, null),
    TASK_ONGOING("EXISTS (SELECT 1 FROM Task task WHERE task.process = ~ "
            + "AND task.processingStatus = :queryObject AND task.title = :#)",
            "~.processingStatus = :queryObject AND ~.title", LikeSearch.NO,
            "tasks AS task WITH task.processingStatus = :queryObject AND task.id",
            "processingStatus = :queryObject AND id", TaskStatus.INWORK, null),
    TASK_FINISHED("EXISTS (SELECT 1 FROM Task task WHERE task.process = ~ "
            + "AND task.processingStatus = :queryObject AND task.title = :#)",
            "~.processingStatus = :queryObject AND ~.title", LikeSearch.NO,
            "tasks AS task WITH task.processingStatus = :queryObject AND task.id",
