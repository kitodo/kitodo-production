...
/**
 * A use folder is a folder in the file system below the process folder that
 ...
 * as computer-readable files in a use folder on the volume. They can have
 * different formats, both in terms of the data type and their form of
 ...
 */
public class UseFolder {
    /**
     * The general metrics of this kind of use folder. So to say its type.
     */
    private final Folder folder;

    /**
     * The process this use folder belongs to. That is the process whose process
    ...
    /**
     * Convenience function to create a bunch of use folders in one.
     * 
     * @param process
     *            the process this use folder belongs to
     * @param folders
     *            The general metrics of the kinds of use folders to create
     * @return use folders
     */
    public static List<UseFolder> createAll(Process process, Collection<Folder> folders) {
        return folders.parallelStream().map(λ -> new UseFolder(process, λ)).collect(Collectors.toList());
    }

    /**
     * Creates a new use folder.
     * 
     * @param process
     *            the process this use folder belongs to
     * @param folder
     *            the general metrics of this kind of use folder
     */
    public UseFolder(Process process, Folder folder) {
    ...
    /**
     * Returns the settings of the use folder configured in the project.
     * 
     * @return the folder
     */
    ...
    private static Map<String, URI> listDirectory(Pair<URI, Pattern> query) {
        KitodoServiceLoader<FileManagementInterface> fileManagementInterface 
                = new KitodoServiceLoader<>(FileManagementInterface.class);
        FilenameFilter filter = (dir, name) -> query.getRight().matcher(name).matches();
        Stream<URI> relativeURIs = fileManagementInterface.loadModule().getSubUris(filter, query.getLeft())
                .parallelStream();
        Stream<URI> absoluteURIs = relativeURIs
                .map(λ -> new File(FilenameUtils.concat(ConfigCore.getKitodoDataDirectory(), λ.getPath())).toURI());
