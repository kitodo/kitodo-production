...
public class CalendarForm {
	...
	public static class Cell {
		 ...
		 * followed by a full stop, as read-only property "day". For cells which are undefined by the  calendar system,
		...
		/**
		 * The function getStyleClass returns the CSS class names to be printed into the HTML to display the table cell
		 * state as read-only property “styleClass”.
		 *
		...
		/**
		 * The method setOnBlock() can be used to change the piece of information whether the day is covered by the
		 * currently showing block or not.
		 *
	...
	/**
	 * The class IssueController backs the control elements that are necessary to manage the properties of an issue
	 * using Faces, including the option of its deletion.
	 *
	 * @author Matthias Ronge &lt;matthias.ronge@zeutschel.de&gt;
	 */
	public class IssueController {
		...
		/**
		 * The method getColour() returns a colour representative for optically distinguishing the given issue as
		 * read-only property "colour".
		 *
		...
		/**
		 * The function getFriday() returns whether the issue held by this controller regularly appears on Fridays as
		 * read-write property "friday".
		 *
		...
		/**
		 * The function getMonday() returns whether the issue held by this controller regularly appears on Mondays as
		 * read-write property "monday".
		 *
		...
		/**
		 * The function getSunday() returns whether the issue held by this controller regularly appears on Sundays as
		 * read-write property "sunday".
		 *
		...
		/**
		 * The function getTuesday() returns whether the issue held by this controller regularly appears on Tuesdays as
		 * read-write property "tuesday".
		 *
		...
		/**
		 * The function getWednesday() returns whether the issue held by thiscontroller regularly appears on Wednesdays
	...
	public static class IssueOption {
		...
		/**
		 * The function getColour() returns a colour representative for optically distinguishing the given issue as
		 * read-only property “colour”.
		 *
		 * @return the HTML colour code of the issue
		 */
		...
		/**
		 * The function getSelected() returns whether the issue appeared on the given date as read-only property
		 * “selected”, taking into consideration the daysOfWeek of regular appearance, the Set of exclusions and
		...
		/**
		 * The method selectClick() is executed if the user clicks an issue option in unselected state. If this is
		 * an exception, the exception will be removed. Otherwise, an additional issue will be added.
		 */
		...
		/**
		 * The method unselectClick() is executed if the user clicks an issue option in selected state. If this is
		 * regular appearance of that issue, an exception will be added. Otherwise, the additional issue
		 * will be removed.
		 */
	...
	/**
	 * The constant field ISSUE_COLOURS holds the colours used to represent the issues in the calendar editor. It is
	 * populated on form bean creation, so changing the configuration should take effect without need to restart the
	 * servlet container.
	 */
	...
	/**
	 * The constant field START_RELATION hold the date the course of publication of the the German-language “Relation
	 * aller Fürnemmen und gedenckwürdigen Historien”, which is often recognized as the first newspaper, began. If
	...
	/**
	 * The Map blockChangerResolver is populated with the IDs used in the block changer drop down element and the
	 * references to the block objects referenced by the IDs for easily looking them up upon change.
	 */
	 ...
	 * This would cause the subsequent calls to other setter methods to overwrite the values in the newly selected block
	 * with the values of the previously displayed block which come back in in the form that is submitted by the browser
	 * if this is not blocked. Therefore setBlockChangerSelected() sets blockChangerUnchanged to control whether the
	 * other setter methods shall or shall not write the incoming data into the respective fields.
	 */
	protected boolean blockChangerUnchanged = true;

	/**
	 * The field blockShowing holds the block currently showing in this calendar instance. The block held in
	 * blockShowing must be part of the courseobject, too.
	 */
	...
	/**
	 * The field firstAppearanceInToChange is set in the setter method setFirstAppearance to notify the setter method
	 * setLastAppearance that the date of first appearance has to be changed. Java Server Faces tries to update the data
	 * model by sequentially calling two setter methods. By allowing the user to alter both fields at one time this may
	 * lead to an illegal intermediate state in the data model which the latter successfully rejects (which it should).
	 * Imagine the case that one block is from March until September and the second one from October to November. Now
	 * the second block shall be moved to January until February. Setting the start date from October to January will
	 * cause an overlapping state with the other block which is prohibited by definition. Therefore changing the
	 * beginning date must be forwarded to the setter method to change the end date to allow this change, which is
	 * allowed, if taken atomically.
	 */
	...
	/**
	 * The field uploadShowing indicates whether the dialogue box to upload a course of appearance XML description is
	 * showing or not.
	 */
	 ...
	 * <p>
	 * The issue colour presets are samples which have been chosen to provide distinguishability also for users with
	 * red-green color vision deficiency. Arbitrary colours can be defined in goobi_config.properties by setting
	 ...
	 *
	 * @param issueControllers the list of issue controllers in question
	...
	/**
	 * The function checkBlockPlausibility compares the dates entered against some plausibility assumptions and sets
	 * hints otherwise.
	 */
	...
	/**
	 * The function downloadClick() is executed if the user clicks the action link to “export” the calendar data. If
	 * the course of appearance doesn’t yet contain generated processes—which is always the case, except that the
	 * user just came from uploading a data file and didn’t change anything about it—process data will be generated.
	 * Then an XML file will be made out of it and sent to the user’s browser. If the granularity was temporarily added,
	 * it will be removed afterwards so that the user will not be presented with the option to generate processes
	 * “as imported” if he or she never ran an import before.
	 *
	 * <p>Note: The process data will be generated with a granularity of “days” (each day forms one process). This
	 * setting can be changed later after the data has been re-imported, but it will remain if the user uploads the
	 * saved data and then proceeds right to the next page and creates processes with the granularity “as imported”.
	 * However, since this is possible and—as to our knowledge in late 2014, when this was written—this is the  best
	 * option of all, this default has been chosen here.</p>
	 */
	...
	/**
	 * The function getBlank() returns whether the calendar editor is in mint condition, i.e. there is no block defined
	 * yet, as read-only property “blank”.
	 * <p>
	 * Side note: “empty” is a reserved word in JSP and cannot be used as property name.
	 * </p>
	...
	/**
	 * The function getBlockChangerOptions() returns the elements for the block changer drop down element as read only
	 * property "blockChangerOptions". It returns a List of Map with each two entries: "value" and "label". "value"
	 * is the hashCode() in hex of the block which will later be used if the field "blockChangerSelected" is altered to
	 * choose the currently selected block, "label" is the readable description to be shown to the user.
	 *
	...
	/**
	 * The function getBlockChangerSelected() returns the hashCode() value of the block currently selected as read-write
	 * property "blockChangerSelected". If a new block is under edit, returns the empty String.
	 *
	 ...
	 * each line then contains 12 cells representing the months. This isdue to HTML table being produced line by line.
	 *
	...
	/**
	 * The function getCourse() will return the course created with this editor as read-only property "course" to pass
	 * it to the next form
	 *
	...
	/**
	 * The function getEmptySheet() builds the empty calendar sheet with 31 rows of twelve cells with empty objects of
	 * type Cell().
	 *
	...
	/**
	 * The function getFirstAppearance() returns the date of first appearance of the block currently showing as
	 * read-write property "firstAppearance".
	 *
	...
	/**
	 * The function getIssues() returns the list of issues held by the block currently showing as read-only property
	 * "issues".
	 *
	 ...
	 *
	 * @param block block whose issues are to be returned
	 * @return the list of issues
	 */
	...
	/**
	 * The function getLastAppearance() returns the date of last appearance of the block currently showing as
	 * read-write property "lastAppearance".
	 *
	...
	/**
	 * The method hideUploadClick() will be called by Faces if the user clicks the cancel button leave the dialog to
	 * upload a course of appearance XML file.
	 */
	...
	/**
	 * The method navigate() alters the year the calendar sheet is shown for so that something of the current block is
	 * visible to prevent the user from needing to click through centuries manually to get there.
	 */
	...
	/**
	 * The function neglectEmptyBlock() removes an empty block. Usually, an empty block cannot be created. But if the
	 * user clicks the upload dialog button, the form must be submitted, which causes the setters of the form fields to
	 * create one prior to the function call. To stay consistent, it is removed here again.
	 */
	...
	/**
	 * Tries to interpret a string entered by the user as a date as flexible as possible. Supports two-digit years and
	 * imperial date field order (month/day/year). In case of flexible interpretations, hints will be displayed to put
	 * the user on the right track what happened to his input. If the user clicks the link to upload a course of
	 * appearance file, no warning message shall show. Therefore an alternate white-space character (U+00A0) will be
	 * appended to the value string by Javascript on the user side because the setter methods will be called by Faces
	 * before the link action will be executed, but we want to skip the error message generation in that case, too.
	 *
	 * @param value value entered by the user
	 * @param input input element, one of "firstAppearance" or "lastAppearance"
	 ...
	 *
	 * @param sheet calendar sheet to populate
	 */
	...
	/**
	 * The method removeBlockClick() deletes the currently selected block from the course of appearance. If there is
	 * only one block left, the editor  will instead be reset.
	 *
	 * @throws IndexOutOfBoundsException if the block referenced by “blockShowing” isn’t contained in the course
	 * 									of appearance
	 */
	...
	/**
	 * The method setBlockChangerSelected() will be called by Faces to store a new value of the read-write property
	 * "blockChangerSelected". If it is different from the current one, this means that the user selected a
	 * different Block in the block changer drop down element. The event will be used to alter the “blockShowing” field
	 * which keeps the block currently showing. “updateAllowed” will be set accordingly to update the contents
	 * of the current block or to prevent fields containing data from the previously displaying block to overwrite the
	 * data inside the newly selected one.
	 *
	 * @param value hashCode() in hex of the block to be selected
	 */
	...
	/**
	 * The method setFirstAppearance() will be called by Faces to store a new value of the read-write property
	 * "firstAppearance", which represents the the date of first appearance of the block currently showing. The event
	 * will be used to either alter the date of first appearance of the block defined by the “blockShowing” field or,
	 * in case that a new block is under edit, to initially set its the date of first appearance.
	 *
	 * @param firstAppearance new date of first appearance
	 */
	...
	/**
	 * The method setLastAppearance() will be called by Faces to store a new value of the read-write property
	 * "lastAppearance", which represents the the date of last appearance of the block currently showing. The event
	 * will be used to either alter the date of last appearance of the block defined by the “blockShowing” field or,
	 * in case that a new block is under edit, to initially set its the date of last appearance.
	 *
	 * @param lastAppearance new date of last appearance
	 */
	...
	/**
	 * The method setUploadedFile() will be called by Faces to store the new  value of the read-write property
	 * "uploadedFile", which is a reference to the binary data the user provides for upload.
	 *
	 * @param data the UploadedFile object generated by the Tomahawk library
	 */
	 ...
	 * for upload in the window and clicks the button to upload it. Old values of the granularity picker are removed—if
	 * any—so that the screen is reinitialised with the current calendar state next time.
	 */
