...
/**
 * The class GranularityForm provides the screen logic for a JSF page to choose the granularity to split up the course
 * of appearance of a newspaper into Goobi processes.
 *
...
public class GranularityForm {
	...
	/**
	 * The field granularity holds the granularity chosen by the user. It is null initially indicating that the user
	 * didn’t choose anything yet.
	 */
	protected Granularity granularity;

	/**
	 * The field course holds the course of appearance previously created by the calendar form instance that from the
	 * information about the issues that appeared is taken to be shown. This field is a managed property which is
	...
	/**
	 * The field numberOfPages holds the total number of pages of the digitization project guessed by the user. It is
	 * null initially indicating that the user didn’t enter anything yet.
	 */
	protected Long numberOfPages;

	/**
	 * The procedure breakModeClick() is called from the procedures which are called if the user clicks one of the
	 * button to select the granularity level. It sets the granularity to the given BreakMode and triggers the
	...
	/**
	 * The procedure daysClick() is called if the user clicks the button to select the granularity level “days”.
	 * It sets the granularity to BreakMode.DAYS and triggers the recalculation of the breaks in the course
	...
	/**
	 * The procedure createProcessesClick() is called if the user clicks the button to create processes for the course
	 * of appearance. If the underlying ProzesskopieForm is incomplete, the user will be redirected back to the page to
	 * fix the problems and nothing more happens. If everything is fine, the meta data field “PublicationRun”—if
	 * available—is populated with the course of appearance in text form. Then, a long running task to create processes
	 * is prepared and the user will be redirected to the task manager page where it can observe the task progressing.
	 *
	...
	/**
	 * The procedure downloadClick() is called if the user clicks the button to download the course of appearance in
	 * XML format.
	 *
	...
	/**
	 * The function getBatchOptions() returns the granularity levels available to summarize the processes to create
	 * in batches along with their verbal description to be shown to the user. The available values depend on the
	 * granularity value of the global variable &ldquo;granularity&rdquo;: Only granularity values <i>broader than</i>
	 * the chosen option are available. If the year level is chosen, or if &ldquo;granularity&rdquo; isn&rsquo;t set,
	 * only the null value, indicating the function is in &ldquo;off&rdquo; state, is returned along with a verbal
	...
	/**
	 * The function getGranularity() returns the granularity level chosen by the user in lower case as read-only
	 * property “granularity”. If there are no processes—indicating that the user didn’t choose anything yet or didn’t
	 * choose anything again after clicking back in the bread crumbs and altering the course of appearance in a way
	 * that the processes need to be recalculated—it literally returns “null” as String. If there are processes loaded
	 * from a foreign source, it returns “foreign”.
	 *
	 * @return the granularity level chosen by the user
	 */
	...
	/**
	 * The function getIssueCount() returns the number of issues that physically appeared as to the underlying course
	 * of appearance data model as read-only property “issueCount”.
	 *
	...
	/**
	 * The function getLockMessage() returns an empty string if either no limit for the minimal number of pages per
	 * process has been configured, or the limit has been reached, which will allow normal processing. Otherwise, an
	 * error message string is returned, explaining the user how to fix the problem. Quotes are replaced for not to
	 * break the Javascript in action here.
	 *
	...
	/**
	 * The function getNumberOfPages returns the total number of pages of the digitization project guessed and entered
	 * by the user—or null indicating that the user didn’t enter anything yet—as read-write property
	 * “numberOfPages”
	 *
	...
	/**
	 * The function getNumberOfPages returns the number of pages per issue guessed and entered by the user—or null
	 * indicating that the user didn’t enter anything yet—as read-write property “numberOfPagesPerIssue”
	 *
	 * @return the total number of pages of the digitization project
	 */
	...
	/**
	 * The function getNumberOfPages returns the total number of pages of the digitization project entered by the user
	 * or a guessed value as read-only property “numberOfPagesOptionallyGuessed”
	 *
	...
	/**
	 * The function getNumberOfProcesses() returns the number of processes that will be created. If the course had to
	 * condemn its processes because the user changed the course, the recalculation of the processes will be
	 * re-initiated here.
	 *
	 ...
	 *
	 * <p>This should be done in JSF using “convertNumber”, but it doesn’t show any effect and the number still prints
	 * with a decimal fractions part that is not desired. This JSF code should put the number rounded into the request
	 * scope, but rounding doesn’t work for some unknown reason.</p>
	 *
	 * <pre>
	...
	/**
	 * The function getSelectedBatchOption() returns the level for which batches will be created as read-write
	 ...
	 * It sets the granularity to BreakMode.ISSUES and triggers the recalculation of the breaks in the course of
	 ...
	 * It sets the granularity to BreakMode.MONTHS and triggers the recalculation of the breaks in the course of
	 * appearance data model.
	 */
	...
	/**
	 * The method setCourse() is called by JSF to inject the course data model  into the form. This behaviour is
	 * configured in faces-config.xml
	 *
	 * @param course Course of appearance data model to be used
	 */
	...
	/**
	 * The procedure setNumberOfPagesPerIssue() is called by Faces on postbacks to save the received value of the
	 * read-write property “numberOfPagesPerIssue”.
	 *
	...
	/**
	 * The procedure setSelectedBatchOption() is called by Faces on postbacks to save the received value of the
	 * read-write property “selectedBatchOption”
	 *
	 * @param option Granularity level for which batches will be created
	 */
	 ...
	 * It sets the granularity to BreakMode.WEEKS and triggers the recalculation of the breaks in the course of
	 * appearance data model.
	 */
	 ...
	 * It sets the granularity to BreakMode.YEARS and triggers the recalculation of the breaks in the course of
	 * appearance data model.
	 */
