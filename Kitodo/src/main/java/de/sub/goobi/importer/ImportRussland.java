...
public class ImportRussland {
	...
	protected void Parsen(BufferedReader reader, Prozess inProzess) throws IOException, WrongImportFileException,
		...
		if (!line.equals("+ " + prozessID + " (ProzessID)")) {
			throw new WrongImportFileException("Importfehler: Importdatei gehört zu einem anderen Werk ('" + prozessID
					+ "' <> '" + line + "')");
		}
		...
		if (listHefte != null) {
			for (Iterator<DocStruct> iter = listHefte.iterator(); iter.hasNext();) {
				...
				if (!iter.hasNext() && !artikelGefunden) {
					throw new WrongImportFileException("Parsingfehler: Artikel mit der ZBL-ID wurde nicht gefunden ('"
							+ zblID + "')");
				}
			}
		} else {
