...
/**
 * The class EmptyTask is the base class for worker threads that operate independently to do the work in
 * the background. The name empty task points out that the task doesn’t do anything sensible yet.
 * It is here to be extended.
 *
...
public class EmptyTask extends Thread implements INameableTask {
     ...
     * <dd>The default behaviour: A thread that terminated either normally or abnormally is kept around in memory
     * for a while and then removed automatically. Numeric and temporary limits can be configured.</dd>
     * <dt><code>PREPARE_FOR_RESTART</code></dt>
     * <dd>If the thread was interrupted by a user, replace it by a new one, passing in the state of the old one
     * to be able to continue work.</dd>
     * </dl>
     *
     * @author Matthias Ronge &lt;matthias.ronge@zeutschel.de&gt;
     */
    ...
    /**
     * The constant CATCH_ALL holds an UncaughtExceptionHandler implementation which will automatically be
     * attached to all task threads. Otherwise exceptions might get lost or even bring the runtime to crash.
     */
    ...
    /**
     * The field behaviour defines the behaviour of the TaskKeeper towards the task if it has terminated.
     * Setting this field to DELETE_IMMEDIATELY will also result in the desired behaviour if the task has not
     * yet been started at all.
     */
    private Behaviour behaviour;

    /**
     * The field detail holds a string giving some details about what the thread is doing that do not
     * require translation, i.e. which file is currently processed.
     */
    ...
    /**
     * The field passedAway will be initialised with a time stamp as the thread dies to be able to remove it
     * a defined timespan after it died.
     */
    ...
    /**
     * Calls the copy constructor to create a not-yet-executed replacement copy of that thread object.
     * Every subclass must provide its own copy constructor—which must call super(objectToCopy)—and overload
     * this method to call its own copy constructor.
     *
    ...
    /**
     * The function getBehaviour() returns the instruction how the TaskSitter shall behave towards this task.
     * Usually, the behaviour isn’t set while the task is under normal execution. It can be set by calling
     ...
     * for a while and then removed automatically. Numeric and temporary limits can be configured.</dd>
     * <dt><code>PREPARE_FOR_RESTART</code></dt>
     * <dd>If the thread was interrupted by a user, replace it by a new one, passing in the state of the old one to
     * be able to continue work.</dd>
     * </dl>
     *
     ...
     * been recorded, null is returned.
     *
    ...
    /**
     * The function getException() provides access to the exception that occurred if the thread died abnormally.
     * If no exception has occurred yet or it wasn’t properly recorded, null is returned.
     *
    ...
    /**
     * The function getProgress() returns the progress of the task in percent, i.e. in a range from 0 to 100.
     *
    ...
    /**
     * The function getStateDescription() returns a text string representing the state of the current task
     * as read-only property "stateDescription".
     *
     ...
     * In our interpretation, a thread is deleteable if it is either new or has terminated and is still lounging around.
     *
    ...
    /**
     * This is a sample implementation of run() which simulates a “long running task” but does nothing and
     * just fills up the percentage gauge. It isn’t useful for anything but testing or demonstration purposes.
     *
    ...
    /**
     * The procedure setException can be used to save an exception that occurred and show it in the front end.
     * It will only record the first exception (which is likely to be the source of all the misery) and it will not
     ...
     * on). The name detail should be set once (in the constructor). You may pass in null to reset the name
    ...
    /**
     * The procedure setWorkDetail() may be used to set some detail information that don’t require translation
     * and are helpful when being shown in the front end (such as the name of the entity that is currently being
     * processed by the task). The name detail should be set every time the progress is determined.
     * You may pass in null to remove the detail.
     *
     * <p>I.e., if your task is about creation of OCR for a process, the detail here could be the image file
     * being processed right now.</p>
     *
     * @param detail
    ...
    /**
     * Causes this thread to begin execution; the Java Virtual Machine will create a standalone thread and call
     * the run method of this class. The result is that two threads are running concurrently: the current thread
     * which returns from the call to the start method, and the other thread which executes its run method.
     * In addition, this method override ensures that the thread is properly registered in the task manager and
     * that its uncaught exception handler has been properly set.
     *
