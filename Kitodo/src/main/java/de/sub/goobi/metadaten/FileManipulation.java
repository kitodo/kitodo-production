...
public class FileManipulation {
	...
	public void uploadFile() {
		...
		try {
			...
			String filename = metadataBean.getMyProzess().getImagesDirectory() + currentFolder + File.separator
					+ basename;
			...
			if (metadataBean.getMyProzess().getImagesTifDirectory(false)
					.equals(metadataBean.getMyProzess().getImagesDirectory() + currentFolder + File.separator)) {
				if (logger.isTraceEnabled()) {
	...
	public void downloadFile() {
		...
		try {
			SafeFile[] filesInFolder = new SafeFile(metadataBean.getMyProzess().getImagesDirectory() + currentFolder)
					.listFiles();
	...
	public void exportFiles() {
		...
		SafeFile destination = new SafeFile(fileuploadFolder.getAbsolutePath() + File.separator
				+ metadataBean.getMyProzess().getTitel());
		...
		for (String filename : filenamesToMove) {
			...
			for (String folder : metadataBean.getAllTifFolders()) {
				try {
					SafeFile[] filesInFolder = new SafeFile(metadataBean.getMyProzess().getImagesDirectory() + folder)
							.listFiles();
					for (SafeFile currentFile : filesInFolder) {
				...
				if (useMasterFolder) {
					...
					if (subfolder.getName().startsWith(masterPrefix)) {
						...
						if (subfolder.getName().contains("_")) {
							String folderSuffix = subfolder.getName().substring(
									subfolder.getName().lastIndexOf("_") + 1);
							...
							if (folderName != null) {
								try {
									...
									Collections.sort(sortedList);
									for (SafeFile object : sortedList) {
										if (currentProcess.getImagesTifDirectory(false).equals(
