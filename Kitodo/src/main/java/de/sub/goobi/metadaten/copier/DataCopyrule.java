...
/**
 * The abstract class DataCopyrule defines method signatures to implement a rule which may later be used to
 * modify metadata depending on various conditions, and provides a factory method to create
 * the matching metadata copy rule implementation from a given command string.
 * 
...
public abstract class DataCopyrule {
    ...
    /**
     * The final Map AVAILABLE_RULES maps the operators of the available metadata copyrules to
     * their respective classes. If more metadata copyrules are to be added to this implementation,
     * they will have to be listed named here.
     */
     ...
     *
     * @param command
     *            A space-separated string consisting of subject (aka. patiens), operator (aka. agens)
     *            and (optional) objects (depending on what objects the operator requires).
     * @return a class implementing the metadata copy rule referenced
     * @throws ConfigurationException
     *             if the operator cannot be resolved or the number of arguments doesn’t match
     */
    ...
    /**
     * The method setObjects() is called to pass the rule its objects. The list passed is reliable to
     * the restrictions defined by getMinObjects() and getMaxObjects().
     *
     * @param objects
