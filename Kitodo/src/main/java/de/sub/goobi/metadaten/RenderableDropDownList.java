...
/**
 * Backing bean for a drop-down style select element to edit a single-select metadata renderable by JSF.
 * 
...
public class RenderableDropDownList extends RenderableMetadatum implements RenderableGroupableMetadatum,
    ...
    /**
     * Constructor to create a backing bean for a drop-down select element renderable by JSF to edit
	 * a choose-from kind of metadata with the option to select exactly one value.
     *
     * @param metadataType
    ...
    /**
     * Returns the identifier of the item currently selected in this drop-down list. If multiple items are
     * internally marked as selected (which is possible if the metadata type under edit was bound to
     * a multi-select list box during creation and is later bound to a drop-down list box for editing)
     * the first of them will be selected. If no item has been selected yet the first available item will be selected.
     *
    ...
    /**
     * Uses the passed-in identifier of the item to be selected to find the first items in the item list in order
     * to mark it as selected and to mark all other items in the item list as not selected.
     *
     * @param value
