...
public class MigrationFormIT {
    ...
    public void testTasksAreEqual() throws DAOException {
        ...
        Assert.assertFalse("Lists should have a different size" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        Task taskTwo = new Task();
        taskTwo.setTitle("testTwo");
        tasksToCompare.add(taskTwo);

        Assert.assertFalse("Tasks should have different Titles" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));
        ...
        Assert.assertFalse("scriptPath should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));
        ...
        Assert.assertFalse("TypeMetadata should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeMetadata(false);
        correctTaskOne.setTypeImagesWrite(true);

        Assert.assertFalse("typeImagesWrite should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeImagesWrite(false);
        correctTaskOne.setTypeImagesRead(true);

        Assert.assertFalse("typeImagesRead should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeImagesRead(false);
        correctTaskOne.setTypeAutomatic(true);

        Assert.assertFalse("typeAutomatic should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeAutomatic(false);
        correctTaskOne.setTypeExportDMS(true);

        Assert.assertFalse("TypeExportDMS should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeExportDMS(false);
        correctTaskOne.setTypeAcceptClose(true);

        Assert.assertFalse("TypeAcceptClose should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));

        correctTaskOne.setTypeAcceptClose(false);
        correctTaskOne.setTypeCloseVerify(true);

        Assert.assertFalse("TypeCloseVerify should be different" ,migrationForm.tasksAreEqual(originalTasks,tasksToCompare));
