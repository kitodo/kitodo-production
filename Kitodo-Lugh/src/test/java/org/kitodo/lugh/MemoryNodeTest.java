...
/** Tests {@code org.kitodo.lugh.MemoryNode}. */
public class MemoryNodeTest {
    ...
    public void testAddAll() {
        ...
        MemoryNode mods = new MemoryNode(Mods.MODS);
    ...
    public void testAddLongObjectType() {
        ...
        final MemoryNode mods = new MemoryNode(Mods.MODS);
    ...
    public void testAsUnordered() {
        MemoryNode testNode = new MemoryNode().add(new MemoryNode(Mods.NAME).setValue("Max"))
                .add(new MemoryNode(Mods.NAME).setValue("Moritz")).put(Mets.USE, "foobar");
    ...
    public void testContains() {
        ...
        MemoryNode node = new MemoryNode().add(javac).add(kitodo).put(Mets.TYPE, mods);
    ...
    public void testEntrySet() {
        MemoryNode node = new MemoryNode().put(Mets.TYPE, Mods.MODS);
        ...
        assertThat(entry.getKey(), is(equalTo(Mets.TYPE.getIdentifier())));
        assertThat(entry.getValue(), hasSize(1));
        assertThat(entry.getValue(), containsInAnyOrder(Mods.MODS));
    ...
    public void testEqualsForTwoEqualMemoryNodes() {
        MemoryNode one = new MemoryNode(Mets.METS);
        MemoryNode another = new MemoryNode(Mets.METS);
    ...
    public void testGetByIdentifierString() {
        MemoryLiteral javac = new MemoryLiteral("javac.exe", RDF.PLAIN_LITERAL);
        MemoryNode mods = new MemoryNamedNode("http://localhost/test/nodeID", Mods.MODS);
        MemoryNodeReference kitodo = new MemoryNodeReference("https://www.kitodo.org/");
    ...
    public void testGetByTypeStringStringString() throws LinkedDataException {
        Node physical = new MemoryNode(Mets.DIV).put(Mets.TYPE, "PHYSICAL");
        Node logical = new MemoryNode(Mets.DIV).put(Mets.TYPE, "LOGICAL");
    ...
    public void testGetStringIdentifiableNodeObjectType() throws LinkedDataException {
        Node physical = new MemoryNode().put(Mets.TYPE, "PHYSICAL");
        Node logical = new MemoryNode().put(Mets.TYPE, "LOGICAL");

        Node node = new MemoryNode().put(Mets.DIV, logical).put(Mets.DIV, physical);

        assertThat(node.get(Mets.DIV.getIdentifier(), Mets.TYPE, MemoryLiteral.createLiteral("LOGICAL", "")).node(),
                is(equalTo(logical)));
        assertThat(node.get(Mets.DIV.getIdentifier(), Mets.TYPE, MemoryLiteral.createLiteral("PHYSICAL", "")).node(),
                is(equalTo(physical)));
    }
    ...
    public void testGetStringString() {
        MemoryNode mods = new MemoryNode(Mods.MODS);
        MemoryNode node = new MemoryNode().put(Mets.DIV, mods);
    ...
    public void testGetType1() {
        assertThat(new MemoryNode(Mods.NAME).getType(), is(equalTo(Mods.NAME.getIdentifier())));
    }
    ...
    public void testHasType() {
        MemoryNode node = new MemoryNode(Mods.MODS).put(RDF.TYPE, Mets.MD_WRAP);
    ...
    public void testIsNotEmpty() {
        assertThat(new MemoryNode(Mets.METS).isEmpty(), is(false));
    }
    ...
    public void testMemoryNodeCanBeCreatedWithMemoryNodeReferenceAsType() {
        ...
        assertThat(mets.getType(), is(equalTo(Mets.METS.getIdentifier())));
    }
    ...
    public void testReplace() {
        ...
        MemoryNode node = new MemoryNode(Mods.NAME).add(alice).add(bob).add(charlie);
    ...
    public void testSetValue() {
        MemoryNode node = new MemoryNode(Mods.NAME).put(Mets.TYPE, "LOGICAL")
                .add(new MemoryLiteral("javac.exe", RDF.PLAIN_LITERAL));

        node.setValue("public static void main(String[] args)");

        assertThat(node.getType(), is(Mods.NAME.getIdentifier()));
    ...
    public void testSize() {
        MemoryNode node = new MemoryNode(Mods.NAME); // 1× rdf:type
        MemoryNode alice = new MemoryNode("http://names.example/alice");
