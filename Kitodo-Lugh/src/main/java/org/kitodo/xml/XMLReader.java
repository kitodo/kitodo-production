...
public class XMLReader {
    ...
    private static String getAttributeFromElement(Element element, String attributeURL, String documentNS) {
        String attributeNS = Namespaces.namespaceOf(attributeURL);
    ...
    private static Optional<String> getType(Element element, String documentNS) {
        ...
        if (ns == null) {
            ...
            return Optional.ofNullable(type.equals(RDF.DESCRIPTION.getIdentifier()) ? null : type);
        }
    }
    ...
    private static ObjectType parseLiteralElement(Element element, NodeReference type, String lang, Space space,
        ...
        if (!type.equals(RDF.LANG_STRING)) {
            return storage.newLiteral(value, type);
        } else {
            return storage.newLangString(value, lang);
        }
    }
    ...
    private static Node parseNodeElement(Element element, String lang, Space space, String documentNS, boolean root,
            Storage storage) {
        String name = element.getAttributeNS(Namespaces.namespaceOf(RDF.ABOUT.getIdentifier()),
                Namespaces.localNameOf(RDF.ABOUT.getIdentifier()));
        if (root && name.isEmpty()) {
            name = documentNS;
        }
        Node result = (name == null) || name.isEmpty() ? storage.newNode() : storage.newNamedNode(name);
        ...
        for (Attr attribute : new GetAttributes(element)) {
            ...
            result.replace(type, inASet(storage.newObjectType(attribute.getValue(), lang)));
        }
        ...
        for (org.w3c.dom.Node child : new GetChildren(element)) {
            ...
            if (child instanceof Element) {
                ...
                // or a literal type
                    ...
                    result.replace(RDF.toURL(++count), inASet(
                            parseLiteralElement((Element) child, literalType, lang, space, documentNS, storage)));
                }

                // or the child is an ordered element (represents a node, linked
                // by a numeric relation representing its element index)
            ...
            // or the child is an XML literal
                ...
                if (!value.isEmpty()) {
                    result.replace(RDF.toURL(++count), inASet(storage.newObjectType(value, lang)));
                }
            }
    ...
    private static Set<ObjectType> parseRelationElement(Element element, String lang, Space space, String documentNS,
        ...
        for (org.w3c.dom.Node child : new GetChildren(element)) {
                        ...
                        if (LITERAL_TYPES.contains(literalType)) {
                            ...
                            if (RDF.LANG_STRING.equals(literalType)) {
                                String ownLang = childElement.getAttributeNS(
                                        Namespaces.namespaceOf(XML.LANG.getIdentifier()),
                                        Namespaces.localNameOf(XML.LANG.getIdentifier()));
                                result.add(storage.newLangString(value, !ownLang.isEmpty() ? ownLang : lang));
                            } else {
                                result.add(storage.newLiteral(value, literalType));
                            }
            ...
            // or the child is an XML literal.
                ...
                String value = space.isToPreserve() ? child.getTextContent() : child.getTextContent().trim();
                if (value.length() > 0) {
                    result.add(storage.newObjectType(value, lang));
                }
            }
