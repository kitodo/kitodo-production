...
public class ConfigOpacCatalogue {
	...
	Node executeBeautifier(Node myHitlist) {
		...
		if (!PicaMassImport.getTempDir().equals("") && new File(PicaMassImport.getTempDir()).canWrite()) {
			debugMyNode(myHitlist, FilenameUtils.concat(PicaMassImport.getTempDir(), "opacBeautifyBefore.xml"));
		}
		...
		if (!PicaMassImport.getTempDir().equals("") && new File(PicaMassImport.getTempDir()).canWrite()) {
			debugMyNode(myHitlist, FilenameUtils.concat(PicaMassImport.getTempDir(), "opacBeautifyAfter.xml"));
		}
		return myHitlist;
	}
