 ...
 */
public class Node implements AccessibleObject, NodeType {
    ...
    private class ChildNodeIterator implements Iterator<ObjectType> {
         ...
         */
        Iterator<Entry<String, Set<ObjectType>>> outer;
    ...
    public Result find(Node graphPath) {
        return GraphPath.apply(new Result(this), graphPath);
    }
    ...
    public Result get(String relation) {
        return new Result(edges.get(relation));
    }
    ...
    public boolean matches(ObjectType condition) {
        ...
        for (Entry<String, Set<ObjectType>> entry : ((Node) condition).edges.entrySet()) {
            String relation = entry.getKey();
            Iterator<ObjectType> candidates;
            if (relation.equals(GraphPath.ANY_PREDICATE.getIdentifier())) {
                candidates = iterator();
            } else {
                Set<ObjectType> related = edges.get(relation);
                if (related == null) {
                    return false;
                }
    ...
    public Node put(String relation, String object) {
        this.put(relation, Literal.create(object, null));
        return this;
    }
    ...
    public void toModel(Model model) {
        ...
        for (Entry<String, Set<ObjectType>> ent : edges.entrySet()) {
    ...
    public RDFNode toRDFNode(Model model) {
        Resource subject = createRDFSubject(model);
        for (Entry<String, Set<ObjectType>> entry : edges.entrySet()) {
    ...
    private void toStringRecursive(StringBuilder out, int indent) {
        ...
        for (Entry<String, Set<ObjectType>> e : edges.entrySet()) {
        ...
        for (Entry<String, Set<ObjectType>> x : attributes.entrySet()) {
            for (ObjectType y : x.getValue()) {
                out.append(spc);
                out.append(x.getKey());
                if (y instanceof Node) {
                    out.append(" {\n");
                    ((Node) y).toStringRecursive(out, indent + 2);
                    out.append(spc);
                    out.append("}\n");
                } else {
                    out.append(" = ");
        ...
        for (Entry<Long, Set<ObjectType>> x : elements.entrySet()) {
            for (ObjectType y : x.getValue()) {
                out.append(spc);
                ...
                if (y instanceof Node) {
                    ...
                    out.append(spc);
                    out.append("}\n");
                } else if (y instanceof NodeReference) {
                    out.append(" = ");
