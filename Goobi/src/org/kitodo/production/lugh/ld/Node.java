...
public class Node implements AccessibleObject, NodeType {
    ...
    public Node(String type) {
        if (type != null) {
            ...
            edges.put(RDF.TYPE.getIdentifier(),
                    new LinkedList<>(Arrays.asList(new ObjectType[] { Literal.create(type, null) })));
        }
    }
    ...
    public Node add(ObjectType element) {
        ...
        edges.put(key, new LinkedList<ObjectType>(Arrays.asList(new ObjectType[] { element })));
        return this;
    }
    ...
    public boolean matches(ObjectType condition) {
        ...
        for (Entry<String, Collection<ObjectType>> entry : ((Node) condition).edges.entrySet()) {
            ...
            Collection<ObjectType> conditions = new LinkedList<>(entry.getValue());
