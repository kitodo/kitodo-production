 ...
 * in what form meta-data for describing these structures can be attached.
 ...
 * correlation section. If so, the meta-data keys named in it are added to the
 ...
 * restriction rule, the remaining (if there is no rule, all) meta-data keys are
 ...
 * <li>From here it becomes specific, that is, the rest depends on the meta-data
 * objects and fields to be added. Now the transferred meta-data objects are
 * assigned to the rows of the two tables. If it turns out that meta-data
 * objects exist for which there is no key in the ruleset (a mis-matching
 * ruleset, for example, because the ruleset was changed), “undefined” meta-data
 ...
 * depends on several factors now. If there is a meta-data object but the key
 * has been defined as excluded, the object will be returned in a line without a
 * key. If there is no meta-data object, a row is only generated if it has been
 * added or is always showing. If there are meta-data objects, a line is created
 ...
 * objects. The whole thing works recursively for complex meta-data keys and
