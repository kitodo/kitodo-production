...
public class HistoryJob implements Job {
	private static final Logger logger = Logger.getLogger(HistoryJob.class);
	public final static String JOBNAME = "dailyStorageHistoryManagerUpdate";
	protected Boolean isRunning = false;
	private static HistoryJob historyJob = new HistoryJob();

	private HistoryJob() {
	}

	public static HistoryJob getInstance() {
		return historyJob;
	}

	/***************************************************************************
	 * execute this {@link Job}
	 * 
	 * @throws JobExecutionException
	 **************************************************************************/
	public void execute(JobExecutionContext context) throws JobExecutionException {
		if (getIsRunning() == false) {
			logger.info("Start scheduled History Manager");
			updateHistoryForAllProcesses();
			logger.info("End scheduled History Manager");
		}
	}
	...
	public static Boolean updateHistory(Prozess inProcess) throws IOException, InterruptedException, SwapException, DAOException {
		...
		/* imagesWork */
		Integer numberWork = de.sub.goobi.helper.FileUtils.getNumberOfFiles(new File(inProcess.getImagesTifDirectory()));
		if (updateHistoryEvent(inProcess, HistoryEventType.imagesWorkDiff, numberWork.longValue())) {
			updated = true;
		}

		/* imagesMaster */
		Integer numberMaster = de.sub.goobi.helper.FileUtils.getNumberOfFiles(new File(inProcess.getImagesOrigDirectory()));
	...
	private static Boolean updateHistoryForSteps(Prozess inProcess) {
		...
		for (Schritt step : inProcess.getSchritteList()) {
			...
			if (step.getEigenschaftenSize() > 0) {
				for (Schritteigenschaft prop : step.getEigenschaftenList()) {
					if (prop.getTitel().equals("Korrektur notwendig") || prop.getType() == PropertyType.messageError) {
		...
		if (isHistoryEventDuplicated(inProcess)) {
			isDirty = true;
		}
	...
	private static HistoryEvent addHistoryEvent(Date timeStamp, Integer stepOrder, String stepName, HistoryEventType type, Prozess inProcess) {
		...
		if (!isHistoryContainsEventAlready(he, inProcess)) {
	 ...
	 ***************************************************************************/
	private static Boolean isHistoryContainsEventAlready(HistoryEvent inEvent, Prozess inProcess) {
	...
	public void updateHistoryForAllProcesses() {
		if (historyJob.getIsRunning() == false) {
			logger.info("start history updating for all processes");
			historyJob.setIsRunning(true);
			try {
				...
				new Helper().setFehlerMeldung("Another Exception occured while scheduled storage calculation", e);
	 ...
	 * @return
	 */
	private static Boolean isHistoryEventDuplicated(Prozess inProcess) {
