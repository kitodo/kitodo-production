...
//TODO: Remove @SuppressWarnings("unchecked"), no tricks, please
...
public class ProzessverwaltungForm extends BasisForm {
	...
	private static final Logger logger = Logger
			.getLogger(ProzessverwaltungForm.class);
	...
	public ProzessverwaltungForm() {
		...
		if (login.getMyBenutzer() != null)
			anzeigeAnpassen.put("processDate", login.getMyBenutzer()
					.isConfVorgangsdatumAnzeigen());
		else
	...
	public String Speichern() {
				...
				for (Vorlage vl : myProzess.getVorlagenList()) {
					for (Vorlageeigenschaft ve : vl.getEigenschaftenList()) {
						if (ve.getWert().contains(myProzess.getTitel()))
							ve.setWert(ve.getWert().replaceAll(
									myProzess.getTitel(), myNewProcessTitle));
					}
				}
				...
				for (Werkstueck w : myProzess.getWerkstueckeList()) {
					for (Werkstueckeigenschaft we : w.getEigenschaftenList()) {
						if (we.getWert().contains(myProzess.getTitel()))
							we.setWert(we.getWert().replaceAll(
									myProzess.getTitel(), myNewProcessTitle));
					}
				}
				/* Vorgangstitel */
		...
		try {
			...
			help.setFehlerMeldung("fehlerNichtSpeicherbar", e.getMessage());
		}
		return "";
	}
	...
	public String Loeschen() {
		...
		try {
			...
			help.setFehlerMeldung("ProzessVerwalung.Loeschen", e);
			return "";
		}
	...
	public String ContentLoeschen() {
		deleteMetadataDirectory();
		help.setMeldung("Content deleted");
		return "";
	}
	...
	private void deleteMetadataDirectory() {
		...
		try {
			...
			help.setFehlerMeldung("Can not delete metadata directory", e);
		}
	}
	...
	public String FilterAktuelleProzesse() {
		...
		try {
			...
			help.setFehlerMeldung(
					"ProzessverwaltungForm.FilterAktuelleProzesse", he);
			return "";
		}
	...
	public String FilterVorlagen() {
		...
		try {
			...
			help.setFehlerMeldung("ProzessverwaltungForm.FilterVorlagen", he);
			return "";
		}
	...
	public String NeuenVorgangAnlegen() {
		...
		if (page.getTotalResults() == 1) {
			// System.out.println("nur einer");
			Prozess einziger = (Prozess) page.getListReload().get(0);
			// System.out.println(einziger.getTitel());
			ProzesskopieForm pkf = (ProzesskopieForm) Helper
					.getManagedBeanValue("#{ProzesskopieForm}");
			pkf.setProzessVorlage(einziger);
			return pkf.Prepare();
		} else {
			// System.out.println("mehrere Treffer");
			return "ProzessverwaltungAlle";
		}
	}
	...
	public String FilterAlleStart() {
		...
		try {
			...
			if (modusAnzeige.equals("vorlagen"))
				...
				crit
						.add(Restrictions.eq("istTemplate", Boolean
								.valueOf(false)));
			...
			help.setFehlerMeldung("fehlerBeimEinlesen", he.getMessage());
			return "";
		} catch (NumberFormatException ne) {
			help.setFehlerMeldung("Falsche Suchparameter angegeben", ne
					.getMessage());
			return "";
	...
	// TODO consider export to Filter Classes
	...
	public String ProzessEigenschaftLoeschen() {
		try {
			...
			help.setFehlerMeldung("fehlerNichtLoeschbar", e.getMessage());
		}
		return "";
	}
	...
	public void ExportPdf() {
		...
		try {
			...
			help.setFehlerMeldung(
					"An error occured while trying to export PDF file for: "
							+ myProzess.getTitel(), e);
	...
	public void ExportDMS() {
		...
		try {
			...
			help.setFehlerMeldung(
					"An error occured while trying to export to DMS for: "
							+ myProzess.getTitel(), e);
	...
	public void ExportDMSPage() {
		...
		for (Prozess proz : (List<Prozess>) page.getListReload()) {
			try {
				...
				// without this a new exception is thrown, if an exception caught here doesn't have an
				// errorMessage
				String errorMessage;
				...
				help.setFehlerMeldung("Fehler beim Export von ID " + proz.getId() + ":", errorMessage);
				e.printStackTrace();
				flagError = true;
			}
		}
		if (flagError){
			help.setFehlerMeldung("Export mit Fehlern abgeschlossen.");
		}else{
			help.setMeldung(null, "xml-Export abgeschlossen", "");
		}
	}
		...
		for (Prozess proz : (List<Prozess>) page.getListReload()) {
			if (proz.isSelected()) {
				try {
					...
					help.setFehlerMeldung("Fehler beim Export", e.getMessage());
					e.printStackTrace();
				}
			}
		}
	...
	public String UploadFromHomeAlle() {
		WebDav myDav = new WebDav();
		myDav.UploadFromHomeAlle("fertig/");
		help.setMeldung(null, "alle Verzeichnisse aus Benutzerhome entfernt",
				"fertig/");
		return "";
	}
	...
	public String UploadFromHome() {
		...
		help.setMeldung(null, "Verzeichnis aus Benutzerhome entfernt",
				myProzess.getTitel());
		return "";
	}
	...
	public void DownloadToHome() {
		/*
		 * --------------------- zunächst prüfen, ob dieser Band gerade von
		 * einem anderen Nutzer in Bearbeitung ist und in dessen Homeverzeichnis
		 * abgelegt wurde, ansonsten Download -------------------
		 */
		if (!myProzess.isImageFolderInUse()) {
			WebDav myDav = new WebDav();
			myDav.DownloadToHome(myProzess, 0, false);
		} else {
			help.setMeldung(null, "Verzeichnis " + myProzess.getTitel()
					+ " befindet sich in Bearbeitung durch ", myProzess
	...
	public void DownloadToHomePage() {
		...
		for (Prozess proz : (List<Prozess>) page.getListReload()) {
			/*
			 * --------------------- zunächst prüfen, ob dieser Band gerade von
			 * einem anderen Nutzer in Bearbeitung ist und in dessen
			...
			if (!proz.isImageFolderInUse()) {
				...
				help.setMeldung(null, "Verzeichnis " + proz.getTitel()
						+ " befindet sich in Bearbeitung durch ", proz
						.getImageFolderInUseUser().getNachVorname());
			}
		}
		help.setMeldung(null,
				"Verzeichnisse dieser Seite in Benutzerhome angelegt", "");
	}
	...
	public void DownloadToHomeHits() {
		...
		help.setMeldung(null,
				"Verzeichnisse aller Treffer in Benutzerhome angelegt", "");
	}
	...
	private void stepStatusUp(Prozess proz) throws DAOException {
		for (Schritt step : proz.getSchritteList()) {
			if (step.getBearbeitungsstatusEnum() != StepStatus.DONE) {
				...
				if(step.getBearbeitungsstatusEnum() == StepStatus.DONE) {
					new HelperSchritte().SchrittAbschliessen(step);
				}else {
	...
	/*
	 * @SuppressWarnings("unchecked") public void RusFullExportPage() {
	 * LoginForm login = (LoginForm) Helper.getManagedBeanValue("#{LoginForm}");
	 * for (Prozess proz : (List<Prozess>) page.getListReload()) { try { String
	 * benutzerHome = login.getMyBenutzer().getHomeDir(); RusslandFullExport rus
	 * = new RusslandFullExport(proz); rus.ExportInDirectoryStart(benutzerHome);
	 * } catch (Exception e) {
	 * help.setFehlerMeldung("Fehler beim RusslandFullExport", e.getMessage());
	 * e.printStackTrace(); } } help.setMeldung(null,
	 * "xml-Export abgeschlossen", ""); }
	 */

	/*
	 * @SuppressWarnings("unchecked") public void RusFullExportHits() {
	 * LoginForm login = (LoginForm) Helper.getManagedBeanValue("#{LoginForm}");
	 * for (Prozess proz : (List<Prozess>) page.getCompleteList()) { try {
	 * String benutzerHome = login.getMyBenutzer().getHomeDir();
	 * RusslandFullExport rus = new RusslandFullExport(proz);
	 * rus.ExportInDirectoryStart(benutzerHome); } catch (Exception e) {
	 * help.setFehlerMeldung("Fehler beim RusslandFullExport", e.getMessage());
	 * e.printStackTrace(); } } help.setMeldung(null,
	 * "xml-Export abgeschlossen", ""); }
	 */
	...
	public void setMyWerkstueckEigenschaft(
			Werkstueckeigenschaft myWerkstueckEigenschaft) {
		// Helper.getHibernateSession();
	...
	public String reihenfolgeUp() {
		mySchritt.setReihenfolge(Integer.valueOf(mySchritt.getReihenfolge()
				.intValue() - 1));
		Speichern();
		return Reload();
	}
	...
	public void setProjektAuswahl(Integer inProjektAuswahl) {
		if (inProjektAuswahl.intValue() != 0)
			try {
				...
				help.setFehlerMeldung("Projekt kann nicht zugewiesen werden",
						"");
				e.printStackTrace();
			}
	}
	...
	public List<SelectItem> getProjektAuswahlListe() throws DAOException {
		...
		List<Projekt> temp = new ProjektDAO()
				.search("from Projekt ORDER BY titel");
	...
	@SuppressWarnings("unchecked")
	public void CalcMetadataAndImagesPage() throws IOException,
			InterruptedException, SwapException, DAOException {
		CalcMetadataAndImages(page.getListReload());
	}

	@SuppressWarnings("unchecked")
	public void CalcMetadataAndImagesSelection() throws IOException,
			InterruptedException, SwapException, DAOException {
	...
	@SuppressWarnings("unchecked")
	public void CalcMetadataAndImagesHits() throws IOException,
			InterruptedException, SwapException, DAOException {
		CalcMetadataAndImages(page.getCompleteList());
	}

	private void CalcMetadataAndImages(List<Prozess> inListe)
		...
		for (Prozess proz : inListe) {
			...
			if (tempMetadata == 0) {
				tempMetadata = zaehlen.getNumberOfUghElements(proz,
						CountType.METADATA);
				if (tempMetadata > 0) {
					changed = true;
				}
			...
			if (tempDocstructs == 0) {
				tempDocstructs = zaehlen.getNumberOfUghElements(proz,
						CountType.DOCSTRUCT);
				if (tempDocstructs > 0) {
					changed = true;
				}
			...
			if (tempImg == 0) {
				tempImg = FileUtils.getNumberOfFiles(new File(proz
			...
			ProcessCounterObject pco = new ProcessCounterObject(
					proz.getTitel(), tempMetadata, tempDocstructs, tempImg);
	...
	public void StatisticsStatusVolumes() {
		statisticsManager = new StatisticsManager(
				StatisticsMode.STATUS_VOLUMES, myFilteredDataSource,
				FacesContext.getCurrentInstance().getViewRoot().getLocale());
		statisticsManager.calculate();
	}

	public void StatisticsUsergroups() {
		statisticsManager = new StatisticsManager(StatisticsMode.USERGROUPS,
				myFilteredDataSource, FacesContext.getCurrentInstance()
	...
	public void StatisticsRuntimeSteps() {
		statisticsManager = new StatisticsManager(
				StatisticsMode.SIMPLE_RUNTIME_STEPS, myFilteredDataSource,
				FacesContext.getCurrentInstance().getViewRoot().getLocale());
	}

	public void StatisticsProduction() {
		statisticsManager = new StatisticsManager(StatisticsMode.PRODUCTION,
				myFilteredDataSource, FacesContext.getCurrentInstance()
						.getViewRoot().getLocale());
	}

	public void StatisticsStorage() {
		statisticsManager = new StatisticsManager(StatisticsMode.STORAGE,
				myFilteredDataSource, FacesContext.getCurrentInstance()
						.getViewRoot().getLocale());
	}

	public void StatisticsCorrection() {
		statisticsManager = new StatisticsManager(StatisticsMode.CORRECTIONS,
				myFilteredDataSource, FacesContext.getCurrentInstance()
						.getViewRoot().getLocale());
	}

	public void StatisticsTroughput() {
		statisticsManager = new StatisticsManager(StatisticsMode.THROUGHPUT,
				myFilteredDataSource, FacesContext.getCurrentInstance()
						.getViewRoot().getLocale());
	}

	public void StatisticsProject() {
		statisticsManager = new StatisticsManager(StatisticsMode.PROJECTS,
				myFilteredDataSource, FacesContext.getCurrentInstance()
						.getViewRoot().getLocale());
		statisticsManager.calculate();
	}
	 ...
	 * WriteException { RusslandExport rus = new RusslandExport(myProzess);
	 * rus.ExportStart(); }
	 * 
	 * public void DownloadRusFullExport() throws IOException, ReadException,
	 * InterruptedException, PreferencesException, SwapException, DAOException,
	 * WriteException { RusslandFullExport rus = new
	 * RusslandFullExport(myProzess); rus.ExportStart(); }
	 */

	public void DownloadMultiTiff() throws IOException, InterruptedException,
	...
	public class ProcessCounterObject {
		...
		public ProcessCounterObject(String title, int metadata, int docstructs,
				int images) {
			super();
			this.title = title;
