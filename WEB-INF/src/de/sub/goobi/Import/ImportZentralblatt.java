...
public class ImportZentralblatt {
   ...
   protected void Parsen(BufferedReader reader, Prozess inProzess) throws IOException,
         ...
         MetadataTypeNotAllowedException, WriteException {
      ...
      LinkedList listArtikel = new LinkedList();
      ...
      while ((line = reader.readLine()) != null) {
         ...
         if (line.length() == 0) {
            ...
            if (posTrennzeichen == -1) {
               myLogger
                     .error("Import() - Parsingfehler (kein Doppelpunkt) der Importdatei in der Zeile <br/>"
               ...
               /* wenn es gerade die Heftnummer ist, dann jetzt dem richtigen Heft zuordnen
                * und dieses ggf. noch vorher anlegen */
               if (myLeft.equals("I")) {
      ...
      try {
         ...
         help.setFehlerMeldung("Import abgebrochen: ", e.getMessage());
         e.printStackTrace();
      }
   ...
   private String xmlTauglichkeitPruefen(String text) {
      ...
      for (int i = 0; i < laenge; i++) {
         ...
         if (!isValidXMLChar(c)) {
            ...
            rueckgabe = HtmlTagsMaskieren(text.substring(0, i)) + "<span class=\"parsingfehler\">" + c
                  + "</span>";
            if (laenge > i)
   ...
   /**
    * Funktion für das Ermitteln des richtigen Heftes für einen Artikel
    * Liegt das Heft noch nicht in dem Volume vor, wird es angelegt.
    * Als Rückgabe kommt das Heft als DocStruct
    * 
   ...
   private DocStruct ParsenHeftzuordnung(DocStruct dsPeriodicalVolume, String myRight,
      ...
      List<DocStruct> myList = dsPeriodicalVolume.getAllChildrenByTypeAndMetadataType("PeriodicalIssue",
            "CurrentNo");
...
//            md.setType(mdt);
            ...
            if (!myRight.equals(md.getValue()))
               throw new WrongImportFileException(
                     "Parsingfehler: verschiedene Zeitschriftennamen in der Datei ('" + md.getValue()
                           + "' & '" + myRight + "')");
         }
         return;
      }
...
//            md.setType(mdt);
   ...
   private void ParsenArtikel(DocStruct inStruct, String myLeft, String myRight, boolean istErsterTitel)
       ...
       * AU:   Autor
       * LA:   Sprache
       * NH: 	Namensvariationen
       * CC:  MSC 2000
       * KW: Keywords
       * AN:  Zbl und/oder JFM Nummer
       * P:      Seiten
       * 
       * --------------------------------*/
