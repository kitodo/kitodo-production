...
public class RequestControlFilter implements Filter {
 ...
 public void doFilter(ServletRequest request, ServletResponse response,
  ...
  try {
   ...
   new Helper()
     .setFehlerMeldung(de.sub.goobi.helper.Helper
       .getTranslation("aLazyInitializationErrorOcurred"));
  }

  /*
   * } catch (Exception e) { new Helper()
   * .setFehlerMeldung(de.sub.goobi.helper.Helper
   * .getTranslation("anUnknownErrorOcurred"));
   * 
   * }
   */
  finally {
 ...
 private void releaseQueuedRequest(HttpServletRequest request) {
  ...
  synchronized (getSynchronizationObject(session)) {
   ...
   // too
   // long and result in another request being processed), then clear
   // it
