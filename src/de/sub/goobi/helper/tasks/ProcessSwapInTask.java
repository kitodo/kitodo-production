...
public class ProcessSwapInTask extends LongRunningTask {
   ...
   public void run() {
      ...
      * -------------------*/
      ...
      for (Iterator it = rootOld.getChildren("file").iterator(); it.hasNext();) {
         Element el = (Element) it.next();
         crcMap.put(el.getAttribute("path").getValue(), el.getAttribute("crc32").getValue());
      }
      help.deleteInDir(fileIn);
      ...
      * -------------------*/
      ...
      try {
         ...
         help.copyDirectoryWithCrc32Check(fileOut, fileIn, swapPath.length(), root);
      } catch (IOException e) {
      ...
      * -------------------*/
      ...
      //TODO: Don't use Iterators
      for (Iterator it = root.getChildren("file").iterator(); it.hasNext();) {
         Element el = (Element) it.next();
