 ...
 * @version 15.06.2009
 */
public class HistoryAnalyserJob2 extends AbstractGoobiJob {
	private static final Logger logger = Logger.getLogger(HistoryAnalyserJob2.class);
		...
		for (Schritt step : inProcess.getSchritteList()) {
			...
			if (step.getEigenschaftenSize() > 0) {
				...
				for (Schritteigenschaft prop : step.getEigenschaftenList()) {
					if (prop.getTitel().equals("Korrektur notwendig")) {
