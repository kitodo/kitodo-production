...
public class KitodoJhoveNisoImageMetadataHelper {
     ...
     */
    public static final Map<String, Function<NisoImageMetadata, double[]>> DOUBLE_ARRAY_PROPERTIES_MAP = Map
            .ofEntries(Map.entry("SubjectDistance", (metadata) -> metadata.getSubjectDistance()));
     ...
     */
    public static final Map<String, Function<NisoImageMetadata, Rational[]>> RATIONAL_ARRAY_PROPERTIES_MAP = Map
            .ofEntries(Map.entry("ReferenceBlackWhite", (metadata) -> metadata.getReferenceBlackWhite()),
    ...
    public static final Map<String, Function<NisoImageMetadata, String>> STRING_PROPERTIES_MAP = Map.ofEntries(
        ...
        Map.entry("MimeType", (metadata) -> metadata.getMimeType()), Map.entry("OS", (metadata) -> metadata.getOS()),
     ...
     */
    public static final Map<String, Function<NisoImageMetadata, String[]>> STRING_ARRAY_PROPERTIES_MAP = Map
            .ofEntries(Map.entry("ProcessingActions", (metadata) -> metadata.getProcessingActions()));

    /**
     * Getter functions that return an index into a label array.
     */
    public static final Map<String, Function<NisoImageMetadata, Integer>> SINGLE_LABEL_PROPERTIES_GETTER_MAP = Map
            .ofEntries(Map.entry("AutoFocus", (metadata) -> metadata.getAutoFocus()),
     ...
     */
    public static final Map<String, Function<NisoImageMetadata, int[]>> MULTIPLE_LABEL_PROPERTIES_GETTER_MAP = Map
            .ofEntries(Map.entry("ExtraSamples", (metadata) -> metadata.getExtraSamples()));
    ...
    public static final Map<String, String[]> LABEL_PROPERTIES_LABEL_ARRAY_MAP = Map.ofEntries(
        Map.entry("AutoFocus", NisoImageMetadata.AUTOFOCUS), Map.entry("BackLight", NisoImageMetadata.BACKLIGHT),
        ...
        Map.entry("ExposureProgram", NisoImageMetadata.EXPOSURE_PROGRAM), Map.entry("Flash", NisoImageMetadata.FLASH),
        ...
        Map.entry("SegmentType", NisoImageMetadata.SEGMENT_TYPE), Map.entry("Sensor", NisoImageMetadata.SENSOR),
     ...
     */
    public static final Map<String, Function<NisoImageMetadata, Integer>> INDEXED_LABEL_PROPERTIES_GETTER_MAP = Map
            .ofEntries(Map.entry("ColorSpace", (metadata) -> metadata.getColorSpace()),
    ...
    /**
     * Label arrays for getter functions that return an index into an indexed
     * label array.
     */
    ...
    /**
     * Index arrays for getter functions that return an index into an indexed
     * label array.
     */
    ...
    private static void extractIndexedLabelProperties(NisoImageMetadata metadata, Map<String, String> map) {
        ...
        for (Map.Entry<String, Function<NisoImageMetadata, Integer>> entry : INDEXED_LABEL_PROPERTIES_GETTER_MAP
                .entrySet()) {
    ...
    /**
     * Extract single label properties from niso image metadata and add them to
     * the metadata map.
     * 
     * @param metadata
    ...
    private static void extractSingleLabelProperties(NisoImageMetadata metadata, Map<String, String> map) {
        // label properties
        for (Map.Entry<String, Function<NisoImageMetadata, Integer>> entry : SINGLE_LABEL_PROPERTIES_GETTER_MAP
                .entrySet()) {
            Integer value = entry.getValue().apply(metadata);
            if (Objects.nonNull(value) && value != NisoImageMetadata.NULL) {
    ...
    /**
     * Extract multiple label properties from niso image metadata and add them
     * to the metadata map.
     * 
     * @param metadata
     *            the niso image metadata
    ...
    private static void extractMultipleLabelProperties(NisoImageMetadata metadata, Map<String, String> map) {
        // label properties
        for (Map.Entry<String, Function<NisoImageMetadata, int[]>> entry : MULTIPLE_LABEL_PROPERTIES_GETTER_MAP
                .entrySet()) {
            int[] values = entry.getValue().apply(metadata);
            if (Objects.nonNull(values)) {
                String[] labelArray = LABEL_PROPERTIES_LABEL_ARRAY_MAP.get(entry.getKey());
                String joined = Arrays.stream(values).filter((idx) -> idx >= 0 && idx < labelArray.length)
                        .mapToObj((idx) -> labelArray[idx]).collect(Collectors.joining(","));
                if (!joined.isEmpty()) {
    ...
    /**
     * Extract number properties from niso image metadata and add them to the
     * metadata map.
     * 
     * @param metadata
     *            the niso image metadata
     ...
     * @param rationalToString
     *            the method that is used to convert a single rational to a
     *            string
    ...
    private static String rationalArrayToString(Rational[] array, Function<Rational, String> rationalToString) {
        ...
        try {
            String joined = Arrays.stream(array).filter(Objects::nonNull).map((r) -> rationalToString.apply(r))
    ...
    /**
     * Extract number array properties from niso image metadata and add them to
     * the metadata map.
     * 
     * @param metadata
     *            the niso image metadata
    ...
    private static void extractNumberArrayProperties(NisoImageMetadata metadata, Map<String, String> map) {
        ...
        for (Map.Entry<String, Function<NisoImageMetadata, Rational[]>> entry : RATIONAL_ARRAY_PROPERTIES_MAP
                .entrySet()) {
    ...
    /**
     * Extract string properties from niso image metadata and add them to the
     * metadata map.
     * 
     * @param metadata
     *            the niso image metadata
    ...
    /**
     * Convert niso image metadata into a simple string map by calling all
